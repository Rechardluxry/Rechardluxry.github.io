...
我是卢叙燃，深圳大学控制科学与工程专业硕士研究生，本科就读于深圳大学机器人工程专业。我关注机器人在真实环境中的自主导航、运动控制与具身系统集成，近期工作围绕 `ROS 2/Nav2`、`Fast-LIO2`、足式机器人强化学习控制和 `Unitree B2/Go2` 实机部署展开。

我希望把算法、系统和实机调试串起来：从 `SLAM`、代价地图、全局/局部规划和点云桥接，到 `AMP`、`PPO`、`Sim2Sim` 与足式机器人运动控制，再到 `Jetson AGX Orin`、`Livox MID360`、`RealSense D435`、`Piper` 机械臂等平台上的部署与联调。
...
该项目面向狗臂协同与具身控制实验，目标是打通四足机器人底盘、感知定位、自主导航和上层任务调用链路，为后续具身智能任务提供稳定系统基础。
...
- 接入 `OpenClaw` 智能体控制框架，实现 B2 四足机器人运动控制、目标点下发和二维导航任务调用。

### 小 Pi 双足机器人 AMP 强化学习与 Sim2Sim 验证

2025.03 - 2025.07，高擎机电实习核心项目  
技术栈：`AMP`、`PPO`、`RSL-RL`、`Legged Gym`、`Isaac Gym`、`MuJoCo`、`ROS`、`PyTorch`、`Python`。

该项目围绕双足机器人强化学习运动控制展开，将 `AMP` 模仿学习框架适配到公司小 Pi 双足平台，并在 `MuJoCo` 中完成跨仿真器验证。
...
- 搭建 `Isaac Gym` 到 `MuJoCo` 的 `Sim2Sim` 验证链路，通过 action 置零、obs 置零、固定动作注入和 play 数据回放等方法定位部署问题。
...
该研究关注机器人在社会动态环境中的安全避障问题，将障碍物类别和交互上下文引入安全边界建模，使机器人对不同语义对象采取差异化但仍可执行的避障策略。

- 参与语义安全余量收紧 `MPC-SECBF` 动态避障方法研究，在 EESM 动态几何安全距离基础上叠加类别与交互风险驱动的有界语义安全余量。
- 构建 `SEESM` 分层规划框架，将语义安全度量用于全局运动基元碰撞检测和局部 `MPC-SECBF` 安全约束。
- 当前作为在研工作展示，后续可根据投稿或发表状态补充会议、期刊和个人贡献细节。

### ROS 智慧物流小车与无人智能小车项目

技术栈：`ROS`、`SLAM`、`Navigation`、二维码识别、路径规划、`DWA`、`TEB`、`PID`、`Magician` 机械臂。

本科阶段围绕移动机器人自主导航和系统搭建完成多项实车项目，覆盖从底盘结构、传感器接入、地图构建到任务点导航和机械臂执行的完整链路。

- 在邮件分拣投递物流小车项目中，基于 `ROS` 构建自主多点导航、二维码任务解析、投递顺序优化和机械臂夹取控制流程。
- 在广东省大学生创新训练项目“自主搭建基于 ROS 的无人智能小车”中负责 `SLAM` 与 `Navigation` 导航模块，完成激光雷达建图、代价地图配置、全局/局部规划参数调试和自主导航实车验证。
- 参与从 `ROS` 驱动板、树莓派上位机、`STM32` 下位机到视觉识别和自主导航的完整机器人系统搭建，解决话题匹配、地图构建和导航参数调试问题。
...
- 基于 `AMP_for_hardware`、`Legged Gym` 与 `RSL-RL` 完成小 Pi 双足机器人 `AMP` 模仿学习任务适配，统一 12 自由度关节映射和 `AMP` 观测格式。
- 搭建 `Isaac Gym` 到 `MuJoCo` 的 `Sim2Sim` 验证链路，系统排查 obs/action/tau、关节顺序、动作缩放、控制频率、URDF/XML 映射和 PD 参数差异。
